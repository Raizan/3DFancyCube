// fragment shading of sphere model MAC VERSION
...
init()
{
    ...
    color4 material_specular( 1.0, 0.7, 1.0, 1.0 );
...
/*----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
        ...
        case 'z':
            color4 light_diffuse( 0.0, 1.0, 1.0, 1.0 );
            color4 material_diffuse( 1.0, 0.0, 0.0, 1.0 );
            color4 diffuse_product = light_diffuse * material_diffuse;
            glLightfv(GL_LIGHT0, GL_DIFFUSE, diffuse_product);
            std::cout << "Light Diffuse!" << std::endl;
            break;
    }
    glutPostRedisplay();
}

void MouseSpecial( int button, int state, int x, int y){
    if(button==GLUT_LEFT_BUTTON && state==GLUT_DOWN){
        double posX = (((double)x/256.0) - 1.0) * -1.0;
        double posY = ((double)y/256.0);
        eye.x=posX;
        eye.y=posY;
...
main( int argc, char **argv )
{
    ...
    glutMouseFunc( MouseSpecial );
    
    menu = glutCreateMenu(MenuFunc);
    glutAddMenuEntry("This is Menu", 1);
